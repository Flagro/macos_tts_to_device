...
def test_bark_engine_initialization(mock_bark_imports):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        # Mock the bark import at module level
        with patch.dict("sys.modules", {"bark": MagicMock()}):
...
def test_generate_audio_success(mock_bark_imports):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        # Create mock Bark modules
        mock_bark_module = MagicMock()
        ...
        mock_bark_module.generate_audio = mock_generate

        with patch.dict("sys.modules", {"bark": mock_bark_module}):
            engine = BarkTTSEngine(
